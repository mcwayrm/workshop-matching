...
In this module, "LASSO matching" means:
...
# Example inputs:
# data$treat is binary treatment indicator
# outcome is data$outcome
...
ps_hat <- as.numeric(predict(cvfit, newx = X, s = "lambda.1se", type = "response"))
data$ps_lasso <- pmin(pmax(ps_hat, 1e-6), 1 - 1e-6)
...
att_fit <- lm(outcome ~ treat, data = matched, weights = weights)
summary(att_fit)
```
...
* User-written command may be required depending on setup
* Example logic:
...
psmatch2 treat, pscore(ps_lasso) outcome(outcome) neighbor(1) caliper(0.2)
...
A good propensity model is one that yields balance, not just high AUC.
