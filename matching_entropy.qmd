...
```r
# R example
balance_vars <- c("age", "education", "income", "female")
...
```r
library(ebal)
eb_out <- ebalance(Treatment = df$treatment,
                   X = df[, balance_vars])
```
...
```r
# Control weights
df$weight[df$treatment == 0] <- eb_out$w

# Treated weights (always 1 for ATT)
df$weight[df$treatment == 1] <- 1
```
...
```r
library(cobalt)
bal.tab(treatment ~ age + education + income + female,
        data = df,
        weights = "weight",
        method = "weighting")
```
...
covariates <- c("age", "sex", "education", "income", "region")
X <- data %>%
  filter(treatment == 0) %>%
  select(all_of(covariates)) %>%
  as.matrix()

T_vec <- data %>% pull(treatment)
...
data$weight[data$treatment == 0] <- eb_fit$w

# Verify balance
bal_check <- bal.tab(
  reformulate(covariates, "treatment"),
  data = data,
  weights = "weight",
  method = "weighting",
  stats = c("m", "v"),
...
summary(data$weight[data$treatment == 0])

ggplot(data %>% filter(treatment == 0), 
       aes(x = weight)) +
...
att_model <- lm(outcome ~ treatment, 
                data = data, 
                weights = weight)
...
library(WeightIt)
W <- weightit(treatment ~ age + sex + education + income + region,
              data = data,
              method = "ebal",
              estimand = "ATT")
...
X_control = data.loc[data['treatment'] == 0, covariates].values
X_treated = data.loc[data['treatment'] == 1, covariates].values
...
data.loc[data['treatment'] == 0, 'weight'] = eb_weights
...
balance_table = check_balance(data, covariates, 'treatment', 'weight')
...
print(data.loc[data['treatment'] == 0, 'weight'].describe())
```
...
ebalance treatment age education income i.sex, ///
    targets(1) ///
    generate(eb_weight)
...
* Visualize balance
ebalance plot treatment age education income, ///
    weights(eb_weight)
...
* Compare balance
tebalance summarize age education income, ///
    baseline ///
    weight(eb_weight)

* Estimate treatment effect
reg outcome treatment [aweight=eb_weight], robust
...
* Check weight distribution
summarize eb_weight if treatment == 0, detail

histogram eb_weight if treatment == 0, ///
...
```r
# Summary statistics
summary(df$weight[df$treatment == 0])

# Effective sample size
sum(df$weight[df$treatment == 0])^2 / 
  sum(df$weight[df$treatment == 0]^2)
```
